...
const JSONToObject = <T>(data: string): Nullable<T> => {
    try {
...
const initMessageHandler = (ws: WebSocket) => {
    ws.on('message', (data: string) => {
        const message: Nullable<Message> = JSONToObject<Message>(data);
        if (message === null) {
        ...
        switch (message.type) {
            ...
            case MessageType.RESPONSE_BLOCKCHAIN:
                const receivedBlocks: Nullable<Block[]> = JSONToObject<Block[]>(message.data);
